...
1. 进入 Typecho 后台 > 管理 > 独立页面 > 新增页面
2. 标题填写"分类 & 标签"（或自定义标题），模板选择 **"分类 & 标签"**
3. 缩略名填入 `categories`（可自定义，需与下方配置一致）
4. 进入后台 > 控制台 > 外观 > 设置外观（iCard 设置）
5. 在"侧边栏设置"标签页，找到 **侧边栏分类按钮名称**（如填"分类"）和 **侧边栏分类按钮链接地址**（填 `categories.html`，与第3步缩略名对应）
6. 保存后在侧边栏即可看到分类入口，点击进入分类标签展示页面

> 提示：若不需要显示侧边栏分类入口，将链接地址留空即可。
